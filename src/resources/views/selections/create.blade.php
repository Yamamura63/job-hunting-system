...
@section('content')
    <h1 class="text-4xl p-3">選考登録</h1>
    <div class="p-8">
        <div class="mx-auto max-w-6xl rounded-lg bg-white p-8 shadow flex justify-center">
            <form method="POST" action="{{ route('selection.store') }}" class="w-full max-w-2xl">
                @csrf
                <div class="m-3">
                    ...
                    ：
                    <input type="text" name="step" value="{{ old('step') }}" required
                        placeholder="ex.) 一次面接" class="m-2 border border-gray-500 rounded">
                </div>

                <div class="m-3">
                    <label class="text-lg font-bold">開催日時</label><br>
                    <input type="datetime-local" name="selection_datetime" value="{{ old('selection_datetime') }}"
                        class="m-2 rounded">
                </div>
                ...
                <div class="m-3">
                    ...
                    <label><input type="radio" name="carfare" value="1" @checked(old('carfare', '1') == '1')>有</label>
                    <label><input type="radio" name="carfare" value="0" @checked(old('carfare') == '0')>無</label>
                </div>

                <div class="m-3">
                    <label class="text-lg font-bold">自費交通費</label>
                    <input type="number" name="carfare_price" value="{{ old('carfare_price') }}"
                ...
                <div class="m-3">
                    ...
                    <textarea name="free_memo" rows="4" class="w-full resize-none m-2 border rounded">{{ old('free_memo') }}</textarea>
                </div>

                <div class="m-3">
                    <label class="text-lg font-bold">結果連絡期間</label><br>
                    <input type="text" name="result_period" value="{{ old('result_period') }}"
                        placeholder="2週間以内 / ○月○日頃" class="w-full m-2 border border-gray-500 rounded">
                </div>
