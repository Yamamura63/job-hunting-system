                ...
                try {
                    ...
                    selfPrs.forEach(selfPr => {
                        ...
                        <div class="flex justify-between items-start">
                            <p
                                class="text-xl font-bold border-b inline-block mb-3 w-64 truncate"
                                title="${escapeHtml(selfPr.title)}">
                                ${escapeHtml(selfPr.title)}
                            </p>
                            <div class="flex items-center gap-2 shrink-0 ml-3">
                                <!-- コピー -->
                                <button type="button" class="copy-button cursor-pointer hover:text-blue-500"
                                ...
                                <!-- 削除 -->
                                <button type="button" class="delete-button cursor-pointer hover:text-red-500" data-id="${selfPr.id}">
                                    <svg xmlns="http://www.w3.org/2000/svg" width="1em" height="1em" viewBox="0 0 24 24">
                                        <path fill="currentColor" d="M7 21q-.825 0-1.412-.587T5 19V6q-.425 0-.712-.288T4 5t.288-.712T5 4h4q0-.425.288-.712T10 3h4q.425 0 .713.288T15 4h4q.425 0 .713.288T20 5t-.288.713T19 6v13q0 .825-.587 1.413T17 21zM17 6H7v13h10z"/>
                                    </svg>
                                </button>
                            </div>
                        </div>
                        ...
                        <button id="button-${selfPr.id}" type="button" class="hidden text-blue-500">
                            もっと見る
                        </button>
                    `;
                        ...
                        // もっと見る
                        const body =
                            card.querySelector(`#body-${selfPr.id}`);
                        const moreButton =
                            card.querySelector(`#button-${selfPr.id}`);
                    ...
                    document.querySelectorAll('.delete-button')
                        .forEach(button =>
                            button.addEventListener('click', async () => {
                                ...
                                await window.indexedDBService.deleteData('self_prs', button.dataset.id);
                                await loadSelfPrs();
                            }));
                        }
                    catch (error) {
